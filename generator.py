...
class custom_generator(Sequence):
    def __init__(self, dataframe : DataFrame, batch_size : int):
        """
        Initialise Dataset
        """
        super().__init__()
        ...
        self.list_IDs = dataframe["id"].tolist()
        ...
        self.lag_three_dict = {}
        self.lag_four_dict = {}
        for id, shop_id, item_id, item_cnt, cat_id, data_block_num, lag_one, lag_two, lag_three, lag_four in zip(self.list_IDs, self.dataframe["shop_id"].tolist(), self.dataframe["item_id"].tolist(), self.dataframe["item_cnt_month"].tolist(), self.dataframe["cat_id"].tolist(), self.dataframe["data_block_num"].tolist(), self.dataframe["lag_one"].tolist(), self.dataframe["lag_two"].tolist(), self.dataframe["lag_three"].tolist(), self.dataframe["lag_four"].tolist()):
            ...
            self.lag_one_dict[id] = lag_one
            self.lag_two_dict[id] = lag_two
            self.lag_three_dict[id] = lag_three
            self.lag_four_dict[id] = lag_four
    ...
    def __data_generation(self, list_IDs):
        ...
        for i, ID in enumerate(list_IDs):
            ...
            X[i,4] = self.lag_one_dict[ID]
            X[i,5] = self.lag_two_dict[ID]
            X[i,6] = self.lag_three_dict[ID]
            X[i,7] = self.lag_four_dict[ID]
